...
def RULE_MATCH(state, rules): # Match rule for a givens state
...
def run(n): # run the gagent through n steps
    print('       Current                                 New')
    print('Location     Status      Action      Location       Status')
    for i in range(1, n):
        (location, status) = Sensors() # Sense Environment before action
        print("{:12}{:8s}".format(location, status), end='')
        action = SIMPLE_REFLEX_AGENT(Sensors())
        Actuators(action)
        (location, status) = Sensors() # Sense Environment aftion
